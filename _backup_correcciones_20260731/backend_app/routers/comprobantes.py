...
from app.models.models import Cliente, Usuario, PagoGarantia
...
def _crear_comprobante(data: ComprobanteCreate, http_request: Request, db: Session, usuario: Usuario) -> dict:
    ...
    # Si es una Nota de Crédito vinculada a una Factura/Boleta existente:
    # monto igual o mayor al saldo → se pregunta al usuario si desea anular
    # la factura (endpoint /anular); monto parcial → se reduce el saldo
    # automáticamente, sin necesidad de confirmación.
    anulacion_pendiente = None
    if data.tipo_documento == "Nota de Crédito" and data.documento_relacionado:
        ...
        if factura:
            monto_nc = total
            saldo_factura = float(
                factura.saldo_pendiente if factura.saldo_pendiente is not None
                else (factura.precio_venta or factura.monto or 0)
            )
            if monto_nc + 0.01 < saldo_factura:
                nuevo_saldo = round(saldo_factura - monto_nc, 2)
                factura.saldo_pendiente = max(0.0, nuevo_saldo)
                factura.estado_cobranza = "Pagada" if factura.saldo_pendiente <= 0.01 else "Pago Parcial"
                db.commit()
                aviso = (aviso + " " if aviso else "") + (
                    f"Se redujo el saldo de la factura {factura.numero_factura} en "
                    f"S/ {monto_nc:,.2f} (nuevo saldo: S/ {factura.saldo_pendiente:,.2f})."
                )
            else:
                anulacion_pendiente = {
                    "factura_id":       factura.id,
                    "numero_documento": factura.numero_factura,
                    "monto":            round(saldo_factura, 2),
                }

    cli = db.query(Cliente).filter(Cliente.id == venta.cliente_id).first() if venta.cliente_id else None
    result = _serialize(venta, cli)
    result["aviso"] = aviso
    result["anulacion_pendiente"] = anulacion_pendiente
    return result
...
def _procesar_pdf_individual(contenido: bytes, nombre_archivo: str, db: Session) -> dict:
    ...
    if resultado["tipo_documento"] == "Nota de Crédito":
        print("=== TEXTO COMPLETO DEL PDF ===")
        print(texto)
        print("==============================")
        print(f"Documento relacionado detectado: {resultado['documento_relacionado']}")
...
def enviar_correo(comp_id: int, data: EnviarComprobanteRequest, db: Session = Depends(get_db)):
    ...
    asunto = f"Comprobante de pago {s['numero_documento']} – {empresa.get('nombre_empresa') or 'Gerencial Pro'}"
    ...
    try:
        enviar_email_smtp(
            ...
            from_name=empresa_cfg.smtp_from_name or empresa.get("nombre_empresa") or "Gerencial Pro",
...
def actualizar(comp_id: int, data: ComprobanteUpdate, http_request: Request, db: Session = Depends(get_db),
    ...
    # Si se edita manualmente el documento_relacionado de una Nota de Crédito:
    # 1) libera la factura/boleta anterior (si cambió), recalculando su saldo
    #    según lo ya cobrado en vez de resetearlo a ciegas — mismo criterio
    #    que la reactivación al eliminar una NC (ver DELETE /{comp_id}).
    # 2) anula la nueva factura/boleta referenciada (mismo efecto que /anular).
    # Sin esto, la factura anterior queda "Anulada" para siempre aunque la NC
    # ya no la referencie, y no aparece como pendiente en Cobranzas.
    if documento_relacionado_cambio and venta.tipo_documento == "Nota de Crédito":
        if documento_relacionado_anterior and documento_relacionado_anterior != venta.documento_relacionado:
            factura_anterior = db.query(VentaComercial).filter(
                VentaComercial.numero_factura == documento_relacionado_anterior,
                VentaComercial.tipo_documento.in_(TIPOS_COBRANZA),
                VentaComercial.estado == "Anulada",
            ).first()
            if factura_anterior:
                total_pagado = float(db.query(func.sum(PagoCobranza.monto_pagado)).filter(
                    PagoCobranza.comprobante_id == factura_anterior.id
                ).scalar() or 0)
                precio_total = float(factura_anterior.precio_venta or factura_anterior.monto or 0)
                nuevo_saldo  = round(precio_total - total_pagado, 2)

                factura_anterior.estado = "Activo"
                if nuevo_saldo <= 0.01:
                    factura_anterior.saldo_pendiente = 0.0
                    factura_anterior.estado_cobranza  = "Pagada"
                elif total_pagado > 0:
                    factura_anterior.saldo_pendiente = nuevo_saldo
                    factura_anterior.estado_cobranza  = "Pago Parcial"
                else:
                    factura_anterior.saldo_pendiente = precio_total
                    factura_anterior.estado_cobranza  = "Pendiente"
                if venta.comprobante_relacionado_id == factura_anterior.id:
                    venta.comprobante_relacionado_id = None

        if venta.documento_relacionado:
            factura_nc = db.query(VentaComercial).filter(
                VentaComercial.numero_factura == venta.documento_relacionado,
                VentaComercial.tipo_documento.in_(TIPOS_COBRANZA),
                VentaComercial.estado != "Anulada",
            ).first()
            if factura_nc:
                factura_nc.estado                = "Anulada"
                factura_nc.estado_cobranza        = "Anulada"
                factura_nc.saldo_pendiente        = 0.0
                venta.comprobante_relacionado_id  = factura_nc.id
        else:
            # Se quitó el documento asociado sin poner uno nuevo.
            venta.comprobante_relacionado_id = None
...
def eliminar_comprobantes_masivo(data: EliminarMasivoRequest, http_request: Request,
    ...
    for comp_id in data.ids:
        ...
        try:
            ...
            if venta.tipo_documento == "Nota de Crédito" and venta.comprobante_relacionado_id:
                factura = db.query(VentaComercial).filter(
                    VentaComercial.id == venta.comprobante_relacionado_id
                ).first()
                if factura and factura.estado == "Anulada":
                    total_pagado = float(db.query(func.sum(PagoCobranza.monto_pagado)).filter(
                        PagoCobranza.comprobante_id == factura.id
                    ).scalar() or 0)
                    ...
                    factura_reactivada = factura.numero_factura
    ...
    if venta.tipo_documento == "Nota de Crédito" and venta.comprobante_relacionado_id:
        ...
        if factura and factura.estado == "Anulada":
            ...
            registrar_log(
                ...
                f"Reactivó el comprobante {factura.numero_factura} al eliminar la Nota de Crédito "
                f"{venta.numero_factura} que lo había anulado", ip_de(http_request),
            )
