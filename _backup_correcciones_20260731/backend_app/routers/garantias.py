...
def _proximo_numero_garantia(db: Session) -> str:
    ...
    a eliminar."""
    rows = db.query(Garantia.numero_documento).filter(
        Garantia.numero_documento.isnot(None),
    ).all()
    max_num = 0
    for (nd,) in rows:
...
@router.get("/resumen")
def resumen_garantias(db: Session = Depends(get_db)):
    total_retenido = float(db.query(func.sum(
        func.coalesce(Garantia.monto_pendiente, Garantia.monto)
    )).filter(Garantia.estado.in_(["retenida", "devolucion_parcial"])).scalar() or 0)

    total_devuelto = float(db.query(func.sum(
        func.coalesce(Garantia.monto_devuelto, 0)
    )).scalar() or 0)

    total_ejecutado = float(db.query(func.sum(
        func.coalesce(Garantia.monto_ejecutado, 0)
    )).scalar() or 0)
...
def aplicar_credito(data: AplicarCreditoRequest, http_request: Request, db: Session = Depends(get_db),
    ...
    credito = db.query(CreditoCliente).filter(CreditoCliente.id == data.credito_id).first()
    if not credito:
        raise HTTPException(404, "Crédito no encontrado")

    venta = db.query(VentaComercial).filter(VentaComercial.id == data.venta_id).first()
    if not venta:
...
def crear_garantia(data: GarantiaCreate, http_request: Request, db: Session = Depends(get_db),
    ...
    if data.tipo_documento == "Recibo Interno":
        numero_documento = _proximo_numero_garantia(db)
    else:
...
def eliminar_garantia(garantia_id: int, http_request: Request, db: Session = Depends(get_db),
    ...
    creditos = db.query(CreditoCliente).filter(
