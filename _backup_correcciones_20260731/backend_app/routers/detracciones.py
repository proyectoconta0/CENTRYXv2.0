...
# ── VARIANTE EXPERIMENTAL — NO CONECTADA a /generar-txt ─────────────────────
# Estructura alternativa de 99 bytes propuesta 2026-07-21, sin fuente oficial
# verificada todavía. A diferencia de _linea_detalle() (107 bytes, en
# producción), esta versión no tiene ningún campo de "tipo de comprobante"
# (Factura/Boleta) — el campo que causó el rechazo real documentado del Banco
# de la Nación en la versión de 107 bytes. No usar en /generar-txt hasta
# confirmar contra el documento oficial o una prueba real con el banco.
def _linea_detalle_v2(g: Gasto, db: Session) -> str:
    """Variante de 99 bytes — experimental, no conectada al endpoint."""
    # col01: Tipo de documento de identidad del proveedor (1) — Catálogo SUNAT 06: "6" = RUC
    col01 = "6"

    # col02: RUC proveedor (11)
    col02 = _pad_izq(_solo_digitos(g.numero_documento), 11)

    # col03: Razón social vacía (35 espacios)
    col03 = " " * 35

    # col04: Código bien/servicio (3)
    col04 = _pad_izq(_codigo_bien_servicio(g.concepto_detraccion), 3)

    # col05: Número de cuenta del proveedor (11) — desde Proveedor.numero_cuenta
    proveedor = db.query(Proveedor).filter(
        Proveedor.numero_documento == _solo_digitos(g.numero_documento)
    ).first()
    col05 = _pad_izq(proveedor.numero_cuenta or "", 11) if proveedor else _pad_izq("", 11)

    # col06: Importe detracción, parte entera (14)
    col06 = _pad_izq(str(int(round(float(g.monto_detraccion or 0)))), 14)

    # col07: Decimales, siempre "00" (2)
    col07 = "00"

    # col08: Tipo de operación (2)
    col08 = "01"

    # col09: Período AAAAMMDD (8)
    col09 = g.fecha.strftime("%Y%m%d") if g.fecha else "00000000"

    # col10: Serie del comprobante (4)
    serie, correlativo = _partir_serie_correlativo(g.numero_comprobante)
    col10 = _pad_der(serie[:4], 4)

    # col11: Número correlativo (8)
    col11 = _pad_izq(correlativo[-8:], 8)

    linea = col01 + col02 + col03 + col04 + col05 + col06 + col07 + col08 + col09 + col10 + col11
    assert len(linea) == 99, f"Línea detalle (v2) con {len(linea)} bytes (esperados 99)"
    return linea
