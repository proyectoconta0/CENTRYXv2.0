...
def _filtro_ventas(q, cliente_id: Optional[int] = None, tipo_servicio: Optional[str] = None):
    ...
    servicio) a un query de VentaComercial."""
    if cliente_id:
...
def _suma_ventas(db: Session, desde: date, hasta: date,
                  cliente_id: Optional[int] = None, tipo_servicio: Optional[str] = None) -> float:
    ...
    q = _filtro_ventas(q, cliente_id, tipo_servicio)
...
def _suma_gastos(db: Session, desde: date, hasta: date) -> float:
    ...
    return float(db.query(func.sum(MONTO_GASTO)).filter(
        Gasto.fecha >= desde,
        Gasto.fecha <= hasta,
...
def _suma_gastos_financieros_prestamos(db: Session, desde: date, hasta: date) -> float:
...
def _suma_cobrado(db: Session, desde: date, hasta: date) -> float:
    return float(db.query(func.sum(PagoCobranza.monto_pagado)).filter(
        PagoCobranza.fecha_pago >= desde,
        PagoCobranza.fecha_pago <= hasta,
    ).scalar() or 0)


def _suma_pagado(db: Session, desde: date, hasta: date) -> float:
    return float(db.query(func.sum(PagoGasto.monto_pagado)).filter(
...
def get_kpis(db: Session, desde: date, hasta: date,
              cliente_id: Optional[int] = None, tipo_servicio: Optional[str] = None) -> dict:
    desde_ant, hasta_ant = _rango_anterior_equivalente(desde, hasta)

    ventas      = _suma_ventas(db, desde, hasta, cliente_id, tipo_servicio)
    ventas_ant  = _suma_ventas(db, desde_ant, hasta_ant, cliente_id, tipo_servicio)
    gastos      = _suma_gastos(db, desde, hasta)
    gastos_ant  = _suma_gastos(db, desde_ant, hasta_ant)
    gastos_financieros_prestamos = _suma_gastos_financieros_prestamos(db, desde, hasta)
    ...
    base_ventas     = _suma_base_ventas(db, desde, hasta, cliente_id, tipo_servicio)
    base_ventas_ant = _suma_base_ventas(db, desde_ant, hasta_ant, cliente_id, tipo_servicio)
    ...
    flujo      = _suma_cobrado(db, desde, hasta) - _suma_pagado(db, desde, hasta)
    flujo_ant  = _suma_cobrado(db, desde_ant, hasta_ant) - _suma_pagado(db, desde_ant, hasta_ant)
...
def get_ventas_evolucion(db: Session, hasta: date = None,
    ...
    for i in range(5, -1, -1):
        ...
        total = _suma_ventas(db, date(anio, mes, 1), date(anio, mes, ultimo_dia), cliente_id, tipo_servicio)
...
def get_flujo_caja(db: Session) -> list:
    ...
    for _ in range(6):
        ...
        ingresos = float(db.query(func.sum(VentaComercial.saldo_pendiente)).filter(
            VentaComercial.estado != "Anulada",
        ...
        egresos = float(db.query(func.sum(Gasto.saldo_pendiente)).filter(
...
def get_top_clientes(db: Session, desde: date, hasta: date,
                      cliente_id: Optional[int] = None, tipo_servicio: Optional[str] = None) -> list:
...
def get_cobranza_estado(db: Session, desde: date, hasta: date,
    ...
    ventas = _filtro_ventas(q, cliente_id, tipo_servicio).all()
...
def get_gastos_categoria(db: Session, desde: date, hasta: date) -> list:
    rows = (
        db.query(Gasto.categoria, func.sum(MONTO_GASTO).label("total"))
        .filter(Gasto.fecha >= desde, Gasto.fecha <= hasta, Gasto.afecta_utilidad.isnot(False))
        .group_by(Gasto.categoria)
        .order_by(func.sum(MONTO_GASTO).desc())
        .all()
    )
    total_general = sum(float(r.total or 0) for r in rows) or 1
    return [
...
def get_proyectos_ejecucion(db: Session) -> list:
...
def get_indicadores_kpi(db: Session, desde: date, hasta: date,
    ...
    total_cobrado = _suma_cobrado(db, desde, hasta)
    total_por_pagar = float(db.query(func.sum(Gasto.saldo_pendiente)).filter(
    ...
    empleados_activos = db.query(func.count(Empleado.id)).filter(
...
def get_filtros_opciones(db: Session) -> dict:
    ...
    tipos_servicio = (
        db.query(VentaComercial.tipo_servicio)
        .filter(VentaComercial.tipo_servicio.isnot(None), VentaComercial.tipo_servicio != "")
        .distinct()
        .order_by(VentaComercial.tipo_servicio)
        .all()
    )
    return {
