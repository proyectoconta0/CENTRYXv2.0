...
function NuevoDocumentoModal({ onClose, onCreated }) {
  ...
  async function guardar() {
    setError("");
    if (!tipo.trim())    { setError("El tipo de documento es requerido"); return; }
    if (!prefijo.trim()) { setError("El prefijo es requerido"); return; }
    setGuardando(true);
    try {
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-medium text-gray-600 mb-1 block">Prefijo</label>
