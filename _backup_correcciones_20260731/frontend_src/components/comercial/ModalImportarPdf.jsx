...
export default function ModalImportarPdf({ onClose, onImported }) {
  ...
  const handleRegistrar = async () => {
    setErrorForm("");
    if (!datos.numero_documento.trim())  { setErrorForm("El N° de documento es requerido"); return; }
    if (!datos.ruc_cliente.trim())       { setErrorForm("El RUC del cliente es requerido"); return; }
    if (!datos.razon_social.trim())      { setErrorForm("La Razón Social es requerida"); return; }
    if (!datos.fecha_emision)            { setErrorForm("La fecha de emisión es requerida"); return; }
    ...
    try {
      ...
      if (r.anulacion_pendiente) {
        setConfirmarAnulacion({ anulacion: r.anulacion_pendiente, notaCreditoId: r.id });
      } else {
        onImported();
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      {confirmarAnulacion && (
        <ModalConfirmarAnulacion
          ...
          onResuelto={() => { setConfirmarAnulacion(null); onImported(); }}
        />
      )}
    </div>
  );
