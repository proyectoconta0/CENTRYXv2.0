...
export default function FormNuevaOrden({ mode, orden, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError("");
    if (!form.tipo_servicio)                 { setError("El tipo de servicio es requerido"); return; }
    if (!form.fecha_inicio)                  { setError("La fecha de inicio es requerida"); return; }
    if (!form.fecha_fin_estimada)            { setError("La fecha fin estimada es requerida"); return; }
          ...
          <form id="orden-form" onSubmit={handleSubmit}>
            ...
            <Section n="5" title="Presupuesto">
              <div className="bg-gray-50 rounded-xl p-5 border border-gray-100 space-y-4">
                ...
                <div>
                  <Label>{form.moneda === "USD" ? "Presupuesto (US$) *" : "Presupuesto (S/) *"}</Label>
                  <div className="relative">
