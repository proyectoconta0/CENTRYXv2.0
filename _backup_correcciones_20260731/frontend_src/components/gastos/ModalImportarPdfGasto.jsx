...
export default function ModalImportarPdfGasto({ onClose, onImported }) {
  ...
  const handleRegistrar = async () => {
    setErrorForm("");
    if (!datos.numero_comprobante.trim()) { setErrorForm("El N° de comprobante es requerido"); return; }
    if (!datos.numero_documento.trim())   { setErrorForm("El RUC/documento del proveedor es requerido"); return; }
    if (!datos.proveedor.trim())          { setErrorForm("El nombre del proveedor es requerido"); return; }
    if (!datos.fecha)                     { setErrorForm("La fecha es requerida"); return; }
