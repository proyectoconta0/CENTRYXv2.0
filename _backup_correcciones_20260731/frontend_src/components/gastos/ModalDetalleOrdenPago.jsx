...
import { getDetalleOrdenPago, editarDetalleOrdenPago } from "../../api/comercialApi";
...
// Modal "Ver Detalle" de una Orden de Pago — mismo estilo visual que
// ModalDetalleLoteDetraccion (lotes de detracciones), pero con edición en
// línea del monto pagado por factura, que este flujo sí necesita.
export default function ModalDetalleOrdenPago({ ordenId, onClose, onChange }) {
  ...
  const cargar = useCallback(async (id) => {
    setLoading(true);
    try { setOrden(await getDetalleOrdenPago(id)); }
    catch { setOrden(null); }
  ...
  const abrirEdicion = (d) => {
    setEditingId(d.id);
    setEditValue(String(d.monto_pagado));
    setError("");
  };

  const guardarEdicion = async (d) => {
    const monto = parseFloat(editValue);
    if (isNaN(monto) || monto < 0) { setError("Ingrese un monto pagado válido."); return; }
    setSavingId(d.id);
    try {
      await editarDetalleOrdenPago(ordenId, d.id, { monto_pagado: monto });
      setEditingId(null);
  ...
  const totalPagado = (orden?.detalle || []).reduce((acc, d) => acc + (d.monto_pagado || 0), 0);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[92vh]">
        <div className="flex items-start justify-between p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Orden de pago</p>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          {loading ? (
            ...
            <p className="text-center text-gray-400 text-sm py-8">No se pudo cargar la orden de pago</p>
          ) : (
            <>
              <div className="border border-green-200 bg-green-50 rounded-xl p-4">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                  <div>
                    ...
                    <p className="font-medium text-gray-800">{orden.nombre_proveedor || "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase">Fecha de pago</p>
                    <p className="font-medium text-gray-800">{fmtFecha(orden.fecha_pago)}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-800">{orden.metodo_pago || "—"}</p>
                  </div>
                  <div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        ...
                        <th className="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 uppercase">Monto Pagado</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {(orden.detalle || []).length === 0 ? (
                        <tr><td colSpan={7} className="text-center py-6 text-gray-400 text-sm">Esta orden no tiene facturas</td></tr>
                      ) : orden.detalle.map(d => (
                        <tr key={d.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-2.5 text-right whitespace-nowrap">
                            {editingId === d.id ? (
                              ...
                              <span className="font-semibold text-gray-800">{fmtS(d.monto_pagado)}</span>
                            )}
                          </td>
                          ...
                          <td className="px-4 py-2.5">
                            <div className="flex items-center justify-center gap-1.5">
                              {editingId === d.id ? (
                                ...
                                <button onClick={() => abrirEdicion(d)} title="Editar monto pagado"
                    ...
                    {(orden.detalle || []).length > 0 && (
                      <tfoot className="bg-gray-50 border-t border-gray-200">
                        <tr>
                          <td colSpan={4} className="px-4 py-2.5 text-right font-semibold text-gray-700">Total pagado</td>
                          <td className="px-4 py-2.5 text-right font-bold text-gray-800 whitespace-nowrap">{fmtS(totalPagado)}</td>
                          <td colSpan={2}></td>
                        </tr>
                      </tfoot>
                    )}
