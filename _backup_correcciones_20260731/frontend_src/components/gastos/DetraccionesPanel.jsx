...
export default function DetraccionesPanel() {
  ...
  const revertirPago = async (lote) => {
    if (!window.confirm(
      `Al eliminar este pago:\n` +
      ` ✓ Se eliminará el registro LOTE-${lote.numero_lote}\n` +
      ` ✓ Las ${lote.cantidad} detracciones volverán a aparecer como pendientes de depósito\n\n` +
      `¿Deseas continuar?`
    )) return;
    ...
    try {
      ...
      alert("No se pudo revertir el pago del lote.");
    } finally {
                    ...
                    <div className="flex items-center justify-center gap-2">
                      ...
                      {l.estado === "pagado" ? (
                        <>
                          ...
                          <button onClick={() => revertirPago(l)} disabled={revirtiendoId === l.id} title="Eliminar pago del lote"
