...
export default function FichaCliente() {
  ...
  const handleAbrirEnvioCorreo = async () => {
    ...
    try {
      ...
      const empresaNombre = r.empresa?.nombre_empresa || "Gerencial Pro";
  ...
  const handleComprobanteFile = async (e) => {
    ...
    if (!["pdf","jpg","jpeg","png"].includes(ext)) {
      alert("Solo se aceptan PDF, JPG y PNG"); setUploadingId(null); return;
    }
    if (file.size > 10 * 1024 * 1024) {
      alert("Archivo mayor a 10 MB"); setUploadingId(null); return;
    }
  ...
  const handleEliminarComprobante = async (ventaId) => {
    ...
    catch { alert("Error al eliminar comprobante"); }
