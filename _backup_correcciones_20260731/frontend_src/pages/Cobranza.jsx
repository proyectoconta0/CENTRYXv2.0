...
// Transferencia → Banco + N° Cuenta
// Depósito      → sin campos adicionales
...
export default function Cobranza() {
  ...
  const handleEditarPago = async () => {
    if (!editPagoModal || !detalleModal) return;
    ...
    if (editPagoForm.metodo_pago === "Transferencia") {
  ...
  const handleEliminarPago = async (pagoId) => {
    if (!window.confirm("¿Eliminar este cobro? El saldo y estado de la factura se recalcularán.")) return;
  ...
  const handleEliminarOrdenCobro = async (row) => {
    if (!window.confirm(
      `Al eliminar esta Orden de Cobro ${row.numero_orden}:\n` +
      ` ✓ Se revertirán los cobros de ${row.cantidad_documentos ?? ""} documento${row.cantidad_documentos === 1 ? "" : "s"}\n` +
    ...
    try {
      ...
      alert("No se pudo eliminar la orden de cobro.");
    } finally {
  ...
  const handleEliminarCobroGarantia = async (row) => {
    ...
    if (!window.confirm(
      `⚠️ Este cobro corresponde a una garantía.\n` +
      `Al eliminarlo se eliminará la garantía por completo:\n` +
      ` - Se revertirán las ejecuciones/créditos que haya generado\n` +
      ` - Los saldos de los documentos afectados se restaurarán\n\n` +
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {loading ? (
                        ...
                        <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-center"><EstadoBadge estado={c.estado_cobranza} pagadoConGarantia={c.pagado_con_garantia} /></td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            {p.tipo === "orden_cobro" ? (
                              <div className="flex items-center justify-center gap-1.5">
                                ...
                                <button onClick={() => handleEliminarOrdenCobro(p)} disabled={eliminandoOrdenId === p.orden_id} title="Eliminar"
                              ...
                              <div className="flex justify-center">
                                <button onClick={() => handleEliminarCobroGarantia(p)} disabled={eliminandoGarantiaId === p.id} title="Eliminar"
                                  className="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-40">
                                  <HiTrash className="w-4 h-4" />
                                </button>
                              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="text-xs font-semibold text-gray-700 uppercase">Monto a Cobrar *</label>
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                {[["Cliente", detalleModal.cliente_nombre], ["RUC", detalleModal.ruc_cliente], ["Tipo Documento", detalleModal.tipo_documento], ["Estado", null], ["Monto Total", fmtS(detalleModal.monto_total)], ["Saldo Pendiente", null], ["Fecha Emisión", fmtFecha(detalleModal.fecha_emision)], ["Fecha Vencto.", fmtFecha(detalleModal.fecha_vencimiento)]].map(([lbl, val]) => (
                  <div key={lbl}>
                    ...
                    {lbl === "Estado" ? <div className="mt-0.5"><EstadoBadge estado={detalleModal.estado_cobranza} /></div>
                    ...
                    {historial.map((p, i) => (
                      <div key={p.id || i} className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-2.5 gap-2">
                        ...
                        <div className="flex items-center gap-1 ml-1">
                          ...
                          <button onClick={() => handleEliminarPago(p.id)} title="Eliminar cobro"
      ...
      {editPagoModal && (
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="text-xs font-semibold text-gray-700 uppercase">Monto Cobrado *</label>
                <div className="relative mt-1.5">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-medium">S/</span>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="text-xs font-semibold text-gray-700 uppercase">Motivo del Extorno *</label>
