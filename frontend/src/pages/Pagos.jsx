...
import CamposMetodoPagoGasto, { validarCamposMetodoPago } from "../components/gastos/CamposMetodoPagoGasto";
...
export default function Pagos() {
  ...
  // ── UI ─────────────────────────────────────────────────────────────────────
  // Si se navega desde Gastos → Cuentas por Pagar con el enlace "Ver en Pagos
  // Tributarios →" (location.state?.tab === "tributarios"), esta pestaña se
  // abre directamente en "Pagos Tributarios" en vez de "Lista de Pagos".
  const [activeTab, setActiveTab] = useState(
    location.state?.tab === "tributarios" ? "pagos_tributarios" : "lista_pagos"
  );

  // ── Confirmación de eliminar (reemplaza window.confirm) y avisos (Toast) ───
  const [confirmAccion, setConfirmAccion] = useState(null); // { tipo, id?, row? } | null
  ...
  const pedirConfirmarEliminarPago = (pagoId) => setConfirmAccion({ tipo: "pago", id: pagoId });

  // ── Lista de Pagos: fila de lote de detracciones ────────────────────────────

  const pedirConfirmarEliminarPagoLote = (row) => setConfirmAccion({ tipo: "lote", row });
  const pedirConfirmarEliminarOrdenPago = (row) => setConfirmAccion({ tipo: "orden", row });

  // ── Lista de Pagos: fila de cuota de préstamo ────────────────────────────────
  const pedirConfirmarEliminarPagoCuota = (row) => setConfirmAccion({ tipo: "cuota", row });

  // ── Lista de Pagos: fila de devolución de garantía ───────────────────────────
  const pedirConfirmarEliminarPagoDevolucion = (row) => setConfirmAccion({ tipo: "devolucion", row });

  // Ejecuta la acción pendiente en `confirmAccion` — reemplaza los antiguos
  // window.confirm()/alert() nativos por ConfirmDialog + Toast.
  const ejecutarConfirmAccion = async () => {
    if (!confirmAccion) return;
    setConfirmandoAccion(true);
    try {
      if (confirmAccion.tipo === "pago") {
        setDeletingPagoId(confirmAccion.id);
        await deletePagoGasto(confirmAccion.id);
      } else if (confirmAccion.tipo === "lote") {
        setRevirtiendoLoteId(confirmAccion.row.lote_id);
        await revertirPagoLoteDetraccion(confirmAccion.row.lote_id);
      } else if (confirmAccion.tipo === "orden") {
        setEliminandoOrdenId(confirmAccion.row.orden_id);
        await eliminarOrdenPago(confirmAccion.row.orden_id);
      } else if (confirmAccion.tipo === "cuota") {
        setEliminandoCuotaPagoId(confirmAccion.row.id);
        await deletePagoGasto(confirmAccion.row.id);
      } else if (confirmAccion.tipo === "devolucion") {
        setEliminandoDevolucionId(confirmAccion.row.id);
        await eliminarDevolucionGarantia(confirmAccion.row.id);
      }
      cargarPagos();
      setConfirmAccion(null);
    } catch (err) {
      const mensajesFallback = {
        lote:  "No se pudo eliminar el pago del lote.",
        orden: "No se pudo eliminar la orden de pago.",
      };
      setToast({ message: mensajesFallback[confirmAccion.tipo] || parsearError(err), type: "error" });
    } finally {
      setConfirmandoAccion(false);
      setDeletingPagoId(null);
      setRevirtiendoLoteId(null);
      setEliminandoOrdenId(null);
      setEliminandoCuotaPagoId(null);
      setEliminandoDevolucionId(null);
    }
  };

  const CONFIRM_ACCION_INFO = {
    pago:       { title: "Eliminar pago", message: "¿Eliminar este pago?" },
    lote: {
      title: "Eliminar pago del lote",
      message: confirmAccion?.tipo === "lote"
        ? `Al eliminar este pago:\n` +
          ` ✓ Se eliminará el registro ${confirmAccion.row.numero_comprobante}\n` +
          ` ✓ Las ${confirmAccion.row.cantidad_facturas ?? ""} detracciones de este lote volverán a aparecer como pendientes de depósito`
        : "",
    },
    orden: {
      title: "Eliminar orden de pago",
      message: confirmAccion?.tipo === "orden"
        ? `Al eliminar esta Orden de Pago ${confirmAccion.row.numero_comprobante}:\n` +
          ` ✓ Se revertirán los pagos de ${confirmAccion.row.cantidad_facturas ?? ""} factura${confirmAccion.row.cantidad_facturas === 1 ? "" : "s"}\n` +
          ` ✓ Los saldos pendientes volverán a su estado anterior`
        : "",
    },
    cuota: {
      title: "Eliminar pago de cuota",
      message:
    ...
    devolucion: {
      title: "Eliminar pago de devolución",
      message:
  ...
  const handleRegistrarPagoTrib = async () => {
    ...
    const errorBancario = validarCamposMetodoPago(tribPagoForm);
    if (errorBancario) { setTribPagoError(errorBancario); return; }
  ...
  // ── imprimir Orden de Pago (PDF generado por el backend) ────────────────────
  const handleImprimirOrdenPago = async (ordenId) => {
    const ventana = window.open("", "_blank");
    setImprimiendoOrdenId(ordenId);
    try {
      const blob = await getOrdenPagoPdf(ordenId);
      const url  = window.URL.createObjectURL(blob);
      if (ventana) ventana.location.href = url;
      else window.open(url, "_blank");
      setTimeout(() => window.URL.revokeObjectURL(url), 60000);
      ...
      if (ventana) ventana.close();
      setToast({ message: "No se pudo generar el PDF de la orden de pago", type: "error" });
    } finally {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
                            ...
                            <div className="flex items-center justify-center gap-1">
                              {p.tipo === "lote_detraccion" ? (
                                <>
                                  ...
                                  <button onClick={() => pedirConfirmarEliminarPagoLote(p)} disabled={revirtiendoLoteId === p.lote_id} title="Eliminar"
                                ...
                                <>
                                  ...
                                  <button onClick={() => handleImprimirOrdenPago(p.orden_id)} disabled={imprimiendoOrdenId === p.orden_id} title="Imprimir"
                                  ...
                                  <button onClick={() => pedirConfirmarEliminarOrdenPago(p)} disabled={eliminandoOrdenId === p.orden_id} title="Eliminar"
                                    className="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-40">
                                    <HiTrash className="w-4 h-4" />
                                  </button>
                                </>
                              ) : p.tipo === "devolucion_garantia" ? (
                                <button onClick={() => pedirConfirmarEliminarPagoDevolucion(p)} disabled={eliminandoDevolucionId === p.id} title="Eliminar"
                                  className="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-40">
                                  <HiTrash className="w-4 h-4" />
                                </button>
                              ) : p.tipo === "cuota_prestamo" ? (
                                <button onClick={() => pedirConfirmarEliminarPagoCuota(p)} disabled={eliminandoCuotaPagoId === p.id} title="Eliminar"
                                  className="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-40">
                                  <HiTrash className="w-4 h-4" />
                                </button>
                              ) : (
                                <>
                                  ...
                                  <button onClick={() => pedirConfirmarEliminarPago(p.id)} disabled={deletingPagoId === p.id} title="Eliminar pago"
                                    className="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-40">
                                    <HiTrash className="w-4 h-4" />
                                  </button>
                                </>
          ...
          {activeTab === "pagos_tributarios" && (
            <>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                {/* Corrección 7: enlace cruzado — estos pagos también aparecen como
                    gastos (badge gris "Tributario") en Cuentas por Pagar. */}
      ...
      {tribFormModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm flex flex-col max-h-[92vh]">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              <div>
                <label className="text-xs font-semibold text-gray-700 uppercase">Concepto <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className="text-xs font-semibold text-gray-700 uppercase">Monto (S/) <span className="text-red-500">*</span></label>
                <div className="relative mt-1.5">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-medium">S/</span>
              ...
              <div>
                <label className="text-xs font-semibold text-gray-700 uppercase">Fecha Límite de Pago <span className="text-red-500">*</span></label>
      ...
      <ConfirmDialog
        open={!!confirmAccion}
        title={confirmAccion ? CONFIRM_ACCION_INFO[confirmAccion.tipo]?.title : ""}
        message={confirmAccion ? CONFIRM_ACCION_INFO[confirmAccion.tipo]?.message : ""}
        danger
        confirmLabel={confirmandoAccion ? "Eliminando…" : "Eliminar"}
        onConfirm={ejecutarConfirmAccion}
