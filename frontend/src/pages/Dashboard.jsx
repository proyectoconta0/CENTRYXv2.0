...
export default function Dashboard() {
  ...
  const { data: kpis,        error: kpisErr,        reload: reloadKpis }        = useFetch(fetchKpis,                FALLBACK.kpis);
  const { data: ventas,      error: ventasErr,      reload: reloadVentas }      = useFetch(fetchVentas,              FALLBACK.ventas);
  const { data: flujo,       error: flujoErr,       reload: reloadFlujo }       = useFetch(getFlujoCaja,            FALLBACK.flujo);
  const { data: topC,        error: topCErr,        reload: reloadTopC }        = useFetch(fetchTopClientes,         FALLBACK.topClientes);
  const { data: cobranza,    error: cobranzaErr,    reload: reloadCobranza }    = useFetch(fetchCobranza,            FALLBACK.cobranza);
  const { data: gastosCat,   error: gastosCatErr,   reload: reloadGastosCat }   = useFetch(fetchGastosCat,          FALLBACK.gastos);
  const { data: proyectos,   error: proyectosErr,   reload: reloadProyectos }   = useFetch(getProyectosEjecucion,   FALLBACK.proyectos);
  const { data: indicadores, error: indicadoresErr, reload: reloadIndicadores } = useFetch(fetchIndicadores,        FALLBACK.indicadores);

  // Si cualquiera de los 8 fetches cayó a su valor FALLBACK, los datos en
  // pantalla no son reales — hay que avisarlo y permitir reintentar.
  const hayError = kpisErr || ventasErr || flujoErr || topCErr || cobranzaErr || gastosCatErr || proyectosErr || indicadoresErr;

  const recargarTodo = useCallback(() => {
    reloadKpis(); reloadVentas(); reloadFlujo(); reloadTopC();
    reloadCobranza(); reloadGastosCat(); reloadProyectos(); reloadIndicadores();
  }, [reloadKpis, reloadVentas, reloadFlujo, reloadTopC, reloadCobranza, reloadGastosCat, reloadProyectos, reloadIndicadores]);
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <Header onFilterChange={setRango} deshabilitarFiltros={hayError} />

        <main className="flex-1 overflow-y-auto bg-gray-50 p-6 space-y-6">

          {/* ── Aviso de datos no reales (fallback) ─────────────── */}
          {hayError && (
            <div className="flex items-center justify-between gap-4 flex-wrap bg-red-50 border border-red-300 text-red-800 rounded-xl px-4 py-3">
              <p className="text-sm font-medium">
                ⚠️ Atención: no se pudo conectar al servidor. Los datos mostrados pueden no ser reales.
              </p>
              <button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-1 h-[250px] md:h-[320px]">
              <TopClientes data={topC} esFallback={topCErr} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="h-[250px] md:h-[340px]">
              <ProyectosEjecucion data={proyectos} esFallback={proyectosErr} />
            </div>
          </div>

          {/* ── Indicadores Financieros ─────────────────────────── */}
          <IndicadoresKPI data={indicadores} esFallback={indicadoresErr} />
