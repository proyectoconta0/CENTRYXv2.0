...
import {
  ...
  eliminarConciliacion, actualizarConciliacion, reconciliarMovimiento,
...
export default function FlujoCaja() {
  ...
  const [actualizarMsg, setActualizarMsg]         = useState(null);
  const [loadingActualizar, setLoadingActualizar] = useState(false);
  const [reconciliandoIds, setReconciliandoIds]   = useState(() => new Set());
  const [confirmDesconciliar, setConfirmDesconciliar]         = useState(null); // id del movimiento o null
  const [confirmGuardarPendientes, setConfirmGuardarPendientes] = useState(null); // Nº pendientes o null
  ...
  async function handleGuardarCuentaBanc() {
    if (!cuentaBancForm.banco.trim())         { setCuentaBancError("El banco es obligatorio."); return; }
    if (!cuentaBancForm.numero_cuenta.trim()) { setCuentaBancError("El número de cuenta es obligatorio."); return; }
  ...
  async function handleCancelarPreview() {
    if (!previewConc) return;
    setLoadingCancelPreview(true);
    setConcError(null);
    try {
      await eliminarConciliacion(previewConc.id);
      setPreviewConc(null);
      setImportArchivo(null);
      setPdfArchivo(null);
    } catch (e) {
      setConcError(e?.response?.data?.detail || "Error al cancelar.");
    } finally {
  ...
  function handleDesconciliar(movId) {
    if (!concActiva) return;
    setConfirmDesconciliar(movId);
  }

  async function ejecutarDesconciliar() {
    const movId = confirmDesconciliar;
    setConfirmDesconciliar(null);
    if (!concActiva || !movId) return;
    try {
  ...
  function handleGuardar() {
    ...
    if (pendientes > 0) {
      setConfirmGuardarPendientes(pendientes);
      return;
    }
    ejecutarGuardar();
  }
  ...
  async function ejecutarGuardar() {
    if (!concActiva) return;
    setConfirmGuardarPendientes(null);
  ...
  async function handleActualizarConciliacion() {
    if (!concActiva) return;
    setLoadingActualizar(true);
    setActualizarMsg(null);
    try {
      const res = await actualizarConciliacion(concActiva.id);
      await loadMovimientos(concActiva.id);
      setActualizarMsg({
        tipo: "ok",
        texto: res.conciliados > 0
          ? `✅ ${res.conciliados} movimiento${res.conciliados === 1 ? "" : "s"} conciliado${res.conciliados === 1 ? "" : "s"}`
          : "No se encontraron nuevas coincidencias.",
      });
    } catch (e) {
      setActualizarMsg({ tipo: "error", texto: e?.response?.data?.detail || "No se pudo actualizar la conciliación." });
    } finally {
      setLoadingActualizar(false);
    }
  }

  async function handleReconciliarMovimiento(movId) {
    if (!concActiva) return;
    setReconciliandoIds(prev => new Set(prev).add(movId));
    try {
      const res = await reconciliarMovimiento(movId);
      await loadMovimientos(concActiva.id);
      setActualizarMsg({
        tipo: res.conciliado ? "ok" : "error",
        texto: res.conciliado ? "✅ Movimiento conciliado" : (res.mensaje || "Sin coincidencia exacta."),
      });
    } catch (e) {
      setActualizarMsg({ tipo: "error", texto: e?.response?.data?.detail || "No se pudo buscar coincidencia." });
    } finally {
      setReconciliandoIds(prev => { const next = new Set(prev); next.delete(movId); return next; });
    }
  }
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
                  ...
                  {previewConc && (
                    <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
                      ...
                      {previewConc.resumen && (
                        <div className="space-y-3">
                          <p className="text-xs text-gray-500">
                            Período: <strong className="text-gray-700">{fmtFecha(previewConc.periodo_desde)} — {fmtFecha(previewConc.periodo_hasta)}</strong>
                            {previewConc.numero_cuenta_detectado && (
                              <> · Cuenta: <strong className="text-gray-700">{previewConc.numero_cuenta_detectado}</strong></>
                            )}
                          </p>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            <KpiCard label="Ingresos" color="green"
                              value={fmtS(previewConc.resumen.total_ingresos)}
                              sub={`${previewConc.resumen.total_ingresos_count} registros`} />
                            <KpiCard label="Egresos" color="red"
                              value={fmtS(previewConc.resumen.total_egresos)}
                              sub={`${previewConc.resumen.total_egresos_count} registros`} />
                            <KpiCard label="Total registros" color="blue"
                              value={previewConc.resumen.total_movimientos}
                              sub="movimientos" />
                            <KpiCard label="Diferencia neta" color={previewConc.resumen.diferencia >= 0 ? "green" : "red"}
                              value={fmtS(previewConc.resumen.diferencia)} />
                          </div>
                        </div>
                      )}
                      ...
                      <div className="flex justify-end gap-3">
                        <button onClick={handleCancelarPreview} disabled={loadingConfirm || loadingCancelPreview}
                          className="px-5 py-2.5 border border-gray-300 text-gray-600 text-sm font-semibold rounded-xl hover:bg-gray-50 disabled:opacity-50">
                          {loadingCancelPreview ? "Cancelando…" : "Cancelar"}
                        </button>
                        <button onClick={handleConfirmar} disabled={loadingConfirm || loadingCancelPreview}
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 px-5 py-4 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={handleActualizarConciliacion}
                        disabled={loadingActualizar}
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border border-blue-200 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 disabled:opacity-50"
                      >
                        <HiRefresh className={`w-3.5 h-3.5 ${loadingActualizar ? "animate-spin" : ""}`} />
                        {loadingActualizar ? "Actualizando…" : "Actualizar Conciliación"}
                      </button>
                      <button
                        onClick={() => {
                          setPasoActivo(1);
                  ...
                  {actualizarMsg && (
                    <div className={`flex items-center gap-2 px-4 py-3 rounded-lg border text-sm ${
                      actualizarMsg.tipo === "ok"
                        ? "bg-green-50 border-green-200 text-green-800"
                        : "bg-red-50 border-red-200 text-red-700"
                    }`}>
                      <span className="flex-1">{actualizarMsg.texto}</span>
                      <button onClick={() => setActualizarMsg(null)} className="text-gray-400 hover:text-gray-600">✕</button>
                    </div>
                  )}
                  ...
                  <TablaMovimientos
                    ...
                    onReconciliar={handleReconciliarMovimiento}
                    reconciliandoIds={reconciliandoIds}
                  />
      ...
      <ConfirmDialog
        open={!!confirmDesconciliar}
        title="Desconciliar movimiento"
        message="El movimiento volverá a quedar pendiente de conciliar."
        confirmLabel="Desconciliar"
        onConfirm={ejecutarDesconciliar}
        onCancel={() => setConfirmDesconciliar(null)}
      />

      <ConfirmDialog
        open={!!confirmGuardarPendientes}
        title="Movimientos pendientes"
        message={confirmGuardarPendientes ? `Tienes ${confirmGuardarPendientes} movimiento${confirmGuardarPendientes !== 1 ? "s" : ""} pendiente${confirmGuardarPendientes !== 1 ? "s" : ""}. ¿Deseas guardar de todas formas?` : ""}
        confirmLabel="Guardar de todas formas"
        onConfirm={ejecutarGuardar}
        onCancel={() => setConfirmGuardarPendientes(null)}
      />
