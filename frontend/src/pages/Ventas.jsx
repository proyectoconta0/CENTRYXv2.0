...
export default function Ventas() {
  ...
  const handleEliminarMasivo = async () => {
    ...
    try {
      ...
      setToast({ message: err.response?.data?.detail || "Error al eliminar los comprobantes seleccionados", type: "error" });
    } finally {
                        ...
                        <tr key={c.id} className={`hover:bg-gray-50 transition-colors ${c.estado === "Anulada" ? "opacity-60" : ""}`}>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-center gap-1">
                              ...
                              {c.estado !== "Anulada" && (
                                <button
                                  onClick={() => imprimirComprobanteVenta(c).catch(() => setToast({ message: "No se pudo abrir el comprobante para imprimir", type: "error" }))}
