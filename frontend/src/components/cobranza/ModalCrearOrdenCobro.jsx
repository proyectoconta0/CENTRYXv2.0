...
import { getDocumentosPendientesOrdenCobro, crearOrdenCobro } from "../../api/comercialApi";
import CamposMetodoPagoGasto from "../gastos/CamposMetodoPagoGasto";

const METODOS_COBRO_OC = ["Efectivo", "Transferencia", "Depósito", "Cheque", "Yape o Plin"];
...
// Se usa la clave "metodo_pago" internamente (no "metodo_cobro") para poder
// reutilizar CamposMetodoPagoGasto tal cual, sin adaptador — se traduce a
// "metodo_cobro" recién al armar el payload en handleGenerar().
const FORM_COBRO_DEFAULT = {
  metodo_pago: "Efectivo", banco: "", numero_cuenta: "", numero_cheque: "", numero_operacion: "",
};

// Modal de 2 pasos: elegir documentos pendientes de cobro de TODO el sistema
// (con filtros de período y RUC/cliente, en vivo) con un monto a cobrar
// editable POR documento → método de cobro y confirmación. Genera UN solo
// registro (OrdenCobro) que consolida el cobro de uno o varios documentos,
// incluso de distintos clientes. Mismo patrón que ModalCrearOrdenPago (Gastos).
export default function ModalCrearOrdenCobro({ cuentasBancarias, onClose, onSuccess }) {
  const [step, setStep] = useState(1);

  // ── Paso 1: Documentos pendientes + filtros + monto a cobrar ───────────
  const [filterDesde,  setFilterDesde]  = useState("");
  const [filterHasta,  setFilterHasta]  = useState("");
  const [filterSearch, setFilterSearch] = useState("");
  const [documentos,   setDocumentos]   = useState([]);
  const [loadingDocs,  setLoadingDocs]  = useState(false);
  // `montos` solo contiene entradas para los documentos marcados: { [ventaId]: "texto del input" }
  const [montos,       setMontos]       = useState({});

  // ── Paso 2: Método de cobro ──────────────────────────────────────────────
  const [fechaCobro, setFechaCobro] = useState(hoy());
  const [form,       setForm]       = useState(FORM_COBRO_DEFAULT);
  ...
  const cargarDocumentos = useCallback(async () => {
    ...
    try {
      ...
      const r = await getDocumentosPendientesOrdenCobro(params);
      setDocumentos(r.data || []);
    } catch { setDocumentos([]); }
    finally { setLoadingDocs(false); }
  }, [filterDesde, filterHasta, filterSearch]);
  ...
  const importeTotalACobrar = seleccionados.reduce((acc, d) => acc + (parseFloat(montos[d.id]) || 0), 0);
  ...
  const handleGenerar = async () => {
    if (!fechaCobro) { setError("La fecha de cobro es obligatoria."); return; }
    if (form.metodo_pago === "Transferencia" || form.metodo_pago === "Depósito") {
      if (!form.banco.trim())         { setError("Ingrese el banco para el cobro por transferencia o depósito."); return; }
      if (!form.numero_cuenta.trim()) { setError("Ingrese el N° de cuenta para el cobro por transferencia o depósito."); return; }
    }
    if (form.metodo_pago === "Cheque") {
      if (!form.banco.trim())         { setError("Ingrese el banco para el cobro por cheque."); return; }
      if (!form.numero_cheque.trim()) { setError("Ingrese el N° de cheque para el cobro por cheque."); return; }
    }
    setSaving(true); setError("");
    try {
      await crearOrdenCobro({
        documentos:       seleccionados.map(d => ({ venta_id: d.id, monto_cobrado: parseFloat(montos[d.id]) })),
        fecha_cobro:      fechaCobro,
        metodo_cobro:     form.metodo_pago,
  ...
  // Nombre a mostrar en el resumen: el único cliente si todos los documentos
  // seleccionados son suyos, o "Varios clientes" si no.
  const nombresSeleccionados = [...new Set(seleccionados.map(d => d.cliente_nombre || "—"))];
  const clienteResumen = nombresSeleccionados.length === 1 ? nombresSeleccionados[0] : "Varios clientes";
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl flex flex-col max-h-[92vh]">
        <div className="flex items-start justify-between p-6 border-b border-gray-200">
          <div>
            <h2 className="text-lg font-bold text-gray-800">Crear Orden de Cobro</h2>
            <p className="text-sm text-gray-500 mt-0.5">
              Paso {step} de 2 — {step === 1 ? "Seleccionar documentos" : "Método de cobro"}
            </p>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          ...
          {step === 1 && (
            <div className="space-y-3">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 flex flex-wrap gap-3 items-center">
                ...
                <input type="text" value={filterSearch} placeholder="RUC o nombre del cliente…"
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
                      <tr>
                        ...
                        <th className="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 uppercase">Monto a Cobrar</th>
                      </tr>
                    </thead>
              ...
              <div className="flex justify-end">
                <p className="text-sm font-semibold text-gray-800">
                  IMPORTE TOTAL A COBRAR: <span className="text-blue-700">{fmtS(importeTotalACobrar)}</span>
                </p>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-gray-700 uppercase">Fecha de Cobro <span className="text-red-500">*</span></label>
              ...
              <div>
                ...
                <select value={form.metodo_pago}
                  onChange={e => setForm(() => ({ ...FORM_COBRO_DEFAULT, metodo_pago: e.target.value }))}
                  className="w-full mt-1.5 px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                  {METODOS_COBRO_OC.map(m => <option key={m} value={m}>{m}</option>)}
                </select>
              </div>
              ...
              <div className="border border-gray-200 bg-gray-50 rounded-xl p-4 space-y-1.5 text-sm">
                <div className="flex justify-between"><span className="text-gray-500">Cliente</span><span className="font-medium text-gray-800">{clienteResumen}</span></div>
                ...
                <div className="flex justify-between pt-1.5 border-t border-gray-200">
                  ...
                  <span className="font-bold text-gray-800">{fmtS(importeTotalACobrar)}</span>
                </div>
              </div>
        ...
        <div className="flex gap-3 px-6 pb-6 pt-2 border-t border-gray-200">
          ...
          {step === 2 && (
            <button onClick={handleGenerar} disabled={saving}
              ...
              {saving ? "Generando…" : "Confirmar Orden de Cobro"}
            </button>
          )}
        </div>
      </div>
