...
import { createComprobante, updateComprobante, getClientes, consultarRuc, getProximoCorrelativoComprobante, getFacturasPorCliente, getCuentasBancarias } from "../../api/comercialApi";
...
const OPCIONES_NOTA_DEBITO = [
  { valor: "cargo_adicional", titulo: "Cargo adicional sobre factura", subtitulo: "Un cargo extra que se suma a la factura relacionada." },
  { valor: "interes_mora",    titulo: "Interés por mora",              subtitulo: "Interés generado por pago fuera de la fecha de vencimiento." },
  { valor: "penalidad",       titulo: "Penalidad",                     subtitulo: "Penalidad contractual aplicada sobre la factura." },
];
...
export default function FormComprobante({ mode, comprobante, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    banco_devolucion:            "",
    numero_cuenta_devolucion:    "",
    numero_operacion_devolucion: "",
    tipo_nota_debito:     "",
    ruc_cliente:          "",
  ...
  // Facturas/Boletas del cliente para el selector de "Documento Relacionado"
  // al crear/editar una Nota de Crédito — se cargan al ingresar el RUC (ver
  // buscarPorRuc).
  const [facturasCliente, setFacturasCliente] = useState([]);

  // Cuentas bancarias para el selector de "Cuenta bancaria" de la devolución
  // (solo NC / devolucion_cobro con método Transferencia o Cheque).
  ...
  useEffect(() => {
    if (comprobante) {
      ...
      setForm({
        ...
        tipo_nota_credito:    comprobante.tipo_nota_credito    || "",
        fecha_devolucion:            comprobante.fecha_devolucion            || today,
        metodo_devolucion:           comprobante.metodo_devolucion           || "Efectivo",
        banco_devolucion:            comprobante.banco_devolucion            || "",
        numero_cuenta_devolucion:    comprobante.numero_cuenta_devolucion    || "",
        numero_operacion_devolucion: comprobante.numero_operacion_devolucion || "",
        tipo_nota_debito:     comprobante.tipo_nota_debito     || "",
      ...
      // Precarga las facturas del cliente para que el selector de
      // "Documento Relacionado" muestre la ya vinculada al editar una NC/ND.
      if (NC_ND.includes(comprobante.tipo_documento) && comprobante.ruc_cliente) {
        buscarFacturasCliente(comprobante.ruc_cliente);
      }
    }
  }, [comprobante]);
  ...
  // Autogenerar correlativo AC-XXXXX / RI-XXXXX según el tipo elegido — Nota
  // de Crédito/Débito NO se numeran solas, el usuario ingresa su N°
  // manualmente (ver "N° de Documento" más abajo).
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Al salir de Nota de Crédito/Débito, descarta el listado de facturas
    // del cliente que pudiera haber quedado de un tipo anterior.
    if (!NC_ND.includes(form.tipo_documento)) {
      setFacturasCliente([]);
    }
  }, [form.tipo_documento]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // Facturas/Boletas activas del cliente — alimenta el selector de
  // "Documento Relacionado" al crear/editar una Nota de Crédito.
  const buscarFacturasCliente = async (ruc) => {
    try {
      const data = await getFacturasPorCliente(ruc);
  ...
  // Al elegir una factura en el selector de "Documento Relacionado" (NC):
  // vincula el documento y precarga su monto total (base/IGV se recalculan
  // solos vía el cálculo ya existente sobre precio_venta).
  const seleccionarFactura = (numeroFactura) => {
    set("documento_relacionado", numeroFactura);
    const f = facturasCliente.find(x => x.numero_factura === numeroFactura);
    if (f) set("precio_venta", String(f.precio_venta));
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    setError("");
    if (!form.numero_documento.trim())     { setError("El N° de documento es obligatorio"); return; }
    if (esBoleta) {
      ...
      if (!esRI && !form.ruc_cliente.trim()) { setError("El RUC es obligatorio"); return; }
      if (esFactura && !/^\d{11}$/.test(form.ruc_cliente.trim())) {
        setError("El RUC debe tener exactamente 11 dígitos numéricos"); return;
      }
      if (!form.razon_social_cliente.trim()) { setError("La Razón Social es obligatoria"); return; }
    }
    if (!form.fecha)                       { setError("La fecha de emisión es obligatoria"); return; }
    if (esNC && !form.tipo_nota_credito)   { setError("Selecciona el tipo de Nota de Crédito"); return; }
    if (esND && !form.tipo_nota_debito)    { setError("Selecciona el tipo de Nota de Débito"); return; }
    if (esDevolucionCobro) {
      if (!form.fecha_devolucion)  { setError("La fecha de devolución es obligatoria"); return; }
      if (!form.metodo_devolucion) { setError("El método de devolución es obligatorio"); return; }
      if (form.metodo_devolucion !== "Efectivo" && (!form.banco_devolucion || !form.numero_cuenta_devolucion)) {
        setError("Selecciona la cuenta bancaria de la devolución"); return;
      }
    }
    const esSinIgvSubmit       = form.tipo_documento === ANTICIPO || form.tipo_documento === RECIBO_INTERNO;
    // Factura, Boleta, Nota de Crédito y Nota de Débito ingresan el monto
    // total con IGV (base/IGV se calculan solos) — solo Anticipo/Recibo
    // Interno (esSinIgvSubmit) quedan fuera de esta rama.
    const esFacturaBoletaSubmit = !esSinIgvSubmit;
    ...
    try {
      const payload = {
        ...
        tipo_nota_credito:     esNC ? form.tipo_nota_credito : null,
        tipo_nota_debito:      esND ? form.tipo_nota_debito : null,
        fecha_devolucion:            esDevolucionCobro ? form.fecha_devolucion : null,
        metodo_devolucion:           esDevolucionCobro ? form.metodo_devolucion : null,
        banco_devolucion:            esDevolucionCobro ? (form.banco_devolucion || null) : null,
        numero_cuenta_devolucion:    esDevolucionCobro ? (form.numero_cuenta_devolucion || null) : null,
        numero_operacion_devolucion: esDevolucionCobro ? (form.numero_operacion_devolucion.trim() || null) : null,
      ...
      if (res?.aviso) setToast({ message: res.aviso, type: "info" });
      onSaved();
    } catch (err) {
  ...
  const esNC       = form.tipo_documento === "Nota de Crédito";
  const esND       = form.tipo_documento === "Nota de Débito";
  const esDevolucionCobro = esNC && form.tipo_nota_credito === "devolucion_cobro";
  ...
  // Numeración de secciones. Nota de Crédito/Débito usan un orden propio,
  // distinto del resto (Factura/Boleta/Anticipo/Recibo Interno, que
  // conservan el orden de siempre): Tipo → Fechas → N° Documento (auto) →
  // Cliente (RUC dispara la búsqueda de sus facturas) → Documento
  // Relacionado (selector de esa factura) → Tipo NC/ND → Descripción →
  // Monto. Recibo Interno oculta "Tipo de Servicio" y "Fecha de
  // Vencimiento"; NC/ND no piden "Tipo de Servicio" en absoluto.
  let _sec = 0;
  let nTipo, nFechas, nNumero, nCliente, nRelacionadoNC, nEscenarioNC, nTipoND, nDevolucion, nServicio, nDescripcion, nMontos, nDetraccion;
  if (esNcNd) {
    nTipo         = ++_sec;
    nFechas       = ++_sec;
    nNumero       = ++_sec;
    nCliente      = ++_sec;
    nRelacionadoNC= ++_sec;
    nEscenarioNC  = esNC ? ++_sec : null;
    nTipoND       = esND ? ++_sec : null;
    nDevolucion   = esDevolucionCobro ? ++_sec : null;
    nDescripcion  = ++_sec;
    nMontos       = ++_sec;
  } else {
    nTipo        = ++_sec;
    nNumero      = ++_sec;
    nCliente     = ++_sec;
    nServicio    = esRI ? null : ++_sec;
    nDescripcion = ++_sec;
    nMontos      = ++_sec;
    nFechas      = ++_sec;
    nDetraccion  = esFactura ? ++_sec : null;
  }

  // Fechas — para Factura/Boleta/Anticipo/Recibo Interno va casi al final
  // (orden de siempre); para Nota de Crédito/Débito va justo después de
  // "Tipo de Documento" (ver nFechas más arriba). Un solo bloque de JSX
  // reutilizado en las dos posiciones posibles, ver más abajo.
  const seccionFechas = (
    <Section n={nFechas} title="Fechas" last={!esFactura && !esNcNd}>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <Label>Fecha {!esRI && "de Emisión "}<span className="text-red-500">*</span></Label>
          <input
            type="date"
            value={form.fecha}
            onChange={e => {
          ...
          {isView ? (
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                ...
                {comprobante.tipo_nota_credito && (
                  <ViewField
                    label="Tipo de Nota de Crédito"
                    value={OPCIONES_NOTA_CREDITO.find(o => o.valor === comprobante.tipo_nota_credito)?.titulo || comprobante.tipo_nota_credito}
                  />
                )}
                {comprobante.tipo_nota_debito && (
                  <ViewField
                    label="Tipo de Nota de Débito"
                    value={OPCIONES_NOTA_DEBITO.find(o => o.valor === comprobante.tipo_nota_debito)?.titulo || comprobante.tipo_nota_debito}
                  />
                )}
            ...
            <form id="comp-form" onSubmit={handleSubmit}>
              ...
              {/* NC/ND: Fechas va aquí, justo después de Tipo de Documento */}
              {esNcNd && seccionFechas}
              ...
              <Section n={nCliente} title="Datos del Cliente">
                {esBoleta ? (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label>RUC{!esRI && <span className="text-red-500"> *</span>}</Label>
              ...
              {/* 3b. Documento relacionado (Nota de Crédito y Nota de Débito)
                  — selector de las facturas del RUC ingresado arriba. */}
              {esNcNd && (
                <Section n={nRelacionadoNC} title="Documento Relacionado">
                  <div>
                    <Label>Factura o Boleta relacionada</Label>
                    <select
                      value={form.documento_relacionado}
                      onChange={e => seleccionarFactura(e.target.value)}
                      disabled={facturasCliente.length === 0}
                      className={`${INPUT_CLS} font-mono ${facturasCliente.length === 0 ? "bg-gray-100 text-gray-400 cursor-not-allowed" : ""}`}
                    >
                      <option value="">
                        {facturasCliente.length ? "Seleccionar factura..." : "Ingresa el RUC del cliente para ver sus facturas"}
                      </option>
                      {facturasCliente.map(f => (
                        <option key={f.numero_factura} value={f.numero_factura}>
                          {f.numero_factura} | {fmtFecha(f.fecha)} | {fmtS(f.precio_venta)}
                        </option>
                      ))}
                    </select>
                  </div>
              ...
              {/* 3c. Escenario de la Nota de Crédito */}
              {esNC && (
                <Section n={nEscenarioNC} title="¿Qué tipo de Nota de Crédito es?">
                    ...
                    {OPCIONES_NOTA_CREDITO.map(op => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 rounded-lg border-2 transition-all ${
                          form.tipo_nota_credito === op.valor
              ...
              {/* 3c'. Tipo de Nota de Débito */}
              {esND && (
                <Section n={nTipoND} title="¿Qué tipo de Nota de Débito es?">
                  <div className="space-y-2">
                    {OPCIONES_NOTA_DEBITO.map(op => (
                      <button
                        key={op.valor}
                        type="button"
                        onClick={() => set("tipo_nota_debito", op.valor)}
                        className={`w-full text-left px-4 py-3 rounded-lg border-2 transition-all ${
                          form.tipo_nota_debito === op.valor
              ...
              {/* 3d. Datos de la devolución (solo escenario "devolucion_cobro") */}
              {esDevolucionCobro && (
                <Section n={nDevolucion} title="Datos de la Devolución">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label>Fecha de devolución <span className="text-red-500">*</span></Label>
                      <input
                        type="date"
                        value={form.fecha_devolucion}
                        onChange={e => set("fecha_devolucion", e.target.value)}
                        className={INPUT_CLS}
                      />
                    </div>
                    <div>
                      ...
                      <select
                        value={form.metodo_devolucion}
                        onChange={e => set("metodo_devolucion", e.target.value)}
                        className={`${INPUT_CLS} bg-white`}
                      >
                        {METODOS_DEVOLUCION.map(m => <option key={m} value={m}>{m}</option>)}
                      </select>
                    </div>
                    {form.metodo_devolucion !== "Efectivo" && (
                      <div className="col-span-2">
                        <Label>Cuenta bancaria <span className="text-red-500">*</span></Label>
                        <select
                          value={
                            cuentasBancarias.find(c =>
                              c.banco === form.banco_devolucion && c.numero_cuenta === form.numero_cuenta_devolucion
                            )?.id || ""
                          }
                          onChange={e => {
                            const cb = cuentasBancarias.find(c => String(c.id) === e.target.value);
                            setForm(f => ({
                              ...f,
                              banco_devolucion:         cb?.banco || "",
                              numero_cuenta_devolucion: cb?.numero_cuenta || "",
                            }));
                          }}
                          className={`${INPUT_CLS} bg-white`}
                        >
                    ...
                    <div className="col-span-2">
                      ...
                      <input
                        type="text"
                        value={form.numero_operacion_devolucion}
                        onChange={e => set("numero_operacion_devolucion", e.target.value)}
              ...
              {/* Tipo de servicio — Recibo Interno y Nota de Crédito/Débito no
                  lo piden (NC/ND solo referencian una factura existente) */}
              {!esRI && !esNcNd && (
                ...
                <div className="bg-gray-50 rounded-xl p-5 border border-gray-100 space-y-4">
                  ...
                  {(() => {
                    ...
                    if (esSinIgv) {
                      ...
                      return (
                        <div>
                          <Label>{labelMonto} <span className="text-red-500">*</span></Label>
                          <div className="relative">
                    ...
                    // Factura / Boleta de Venta / Nota de Crédito / Nota de
                    // Débito — se ingresa el monto total con IGV y la
                    // base/IGV se calculan solos.
                    ...
                    const labelTotal = esNcNd
                      ? (esUSD ? "Monto Total con IGV (US$)" : "Monto Total (con IGV)")
                      : (esUSD ? "Precio de Venta (US$)"     : "Precio de Venta (S/)");
                    return (
                      <>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                        ...
                        <div>
                          <Label>{labelTotal} <span className="text-red-500">*</span></Label>
                          <div className="relative">
              ...
              {/* Factura/Boleta/Anticipo/Recibo Interno: Fechas va aquí, casi
                  al final (orden de siempre) */}
              {!esNcNd && seccionFechas}
                    ...
                    {form.tiene_detraccion && (() => {
                      ...
                      return (
                        <div className="space-y-4">
                          ...
                          <div>
                            <Label>Tasa (%){form.concepto_detraccion === "Otro" && <span className="text-red-500"> *</span>}</Label>
                            <input
