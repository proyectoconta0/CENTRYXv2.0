...
import { getDetalleMovimientoConciliacion } from "../../api/comercialApi";
...
function DetalleOrdenCobro({ estado }) {
  if (!estado || estado.loading) {
    return <p className="text-xs text-gray-400 italic">Cargando detalle de la orden de cobro…</p>;
  }
  if (estado.error || !estado.data) {
    return <p className="text-xs text-red-400 italic">No se pudo cargar el detalle de la orden de cobro.</p>;
  }
  const oc = estado.data;
  return (
    <div className="text-xs">
      <div className="flex items-center gap-2 mb-2">
        <span className="w-2 h-2 rounded-full bg-green-500 shrink-0" />
        <span className="font-semibold text-gray-800">Orden de Cobro {oc.numero_orden}</span>
        <span className="text-gray-500">— Cliente: {oc.nombre_cliente || "—"}</span>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full border border-gray-200 rounded-lg overflow-hidden">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-3 py-1.5 text-left font-semibold text-gray-500">N° Doc</th>
              <th className="px-3 py-1.5 text-left font-semibold text-gray-500">Tipo</th>
              <th className="px-3 py-1.5 text-right font-semibold text-gray-500">Cobrado</th>
              <th className="px-3 py-1.5 text-right font-semibold text-gray-500">Saldo</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {oc.detalle.map(d => (
              <tr key={d.id}>
                <td className="px-3 py-1.5 font-mono text-gray-700">{d.numero_documento || "—"}</td>
                <td className="px-3 py-1.5 text-gray-600">{d.tipo_documento || "—"}</td>
                <td className="px-3 py-1.5 text-right text-gray-800">{fmtS(d.monto_cobrado)}</td>
                <td className="px-3 py-1.5 text-right text-gray-500">{fmtS(d.saldo_restante)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="border-t border-gray-200 bg-gray-50 font-semibold">
              <td className="px-3 py-1.5 text-gray-700" colSpan={2}>Total cobrado</td>
              <td className="px-3 py-1.5 text-right text-gray-800">{fmtS(oc.monto_total)}</td>
              <td />
            </tr>
          </tfoot>
        </table>
...
export default function TablaMovimientos({
  ...
  const [detalleOC, setDetalleOC]   = useState({}); // movId -> { loading, error, data }

  function toggleExpand(m) {
    setExpandedId(prev => {
      const next = prev === m.id ? null : m.id;
      if (next && m.referencia_sistema_tipo === "orden_cobro" && !detalleOC[m.id]) {
        setDetalleOC(d => ({ ...d, [m.id]: { loading: true } }));
        getDetalleMovimientoConciliacion(m.id)
          .then(data => setDetalleOC(d => ({ ...d, [m.id]: { loading: false, data } })))
          .catch(() => setDetalleOC(d => ({ ...d, [m.id]: { loading: false, error: true } })));
      }
      return next;
    });
  }
                ...
                <React.Fragment key={m.id}>
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 text-center" onClick={e => e.stopPropagation()}>
                      ...
                      {/* Solo banco (no bancario) → Buscar coincidencia / Ignorar */}
                      {st === "solo_banco" && (
                        <div className="flex items-center justify-center gap-1.5">
                          <button
                            onClick={() => onReconciliar(m.id)}
                            disabled={reconciliandoIds?.has(m.id)}
                            className="text-[10px] px-2 py-1 rounded-lg font-medium bg-blue-100 text-blue-700 hover:bg-blue-200 whitespace-nowrap disabled:opacity-50"
                          >
                            {reconciliandoIds?.has(m.id) ? "Buscando…" : "🔄 Buscar coincidencia"}
                          </button>
                          <button
                  ...
                  {expanded && (
                    <tr>
                      <td colSpan={7} className="px-8 py-3 bg-white border-b border-gray-100">
                        {m.referencia_sistema_tipo === "orden_cobro" ? (
                          <DetalleOrdenCobro estado={detalleOC[m.id]} />
                        ) : m.documento_sistema ? (
