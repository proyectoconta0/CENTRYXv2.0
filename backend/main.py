...
from app.models import empresa as empresa_models            # registra tabla empresas (Multi-Tenant)
...
from app.core.security import require_modulo, get_current_usuario, require_superadmin
...
def _run_migrations():
    with engine.connect() as conn:
        ...
        # Usamos DO block para evitar error si la constraint ya existe.
        conn.execute(text("""
            DO $$
            BEGIN
                BEGIN
...
# ── Migración Multi-Tenant: añade empresa_id a todas las tablas ───────────────
def _run_multitenant_migrations():
    """
    Añade empresa_id (FK → empresas.id, nullable) a todas las tablas que
    participan en el aislamiento por tenant. Idempotente: ADD COLUMN IF NOT EXISTS.
    La tabla 'empresas' ya existe en este punto porque create_all() la creó arriba.
    """
    tablas = [
        "usuarios", "clientes", "gastos", "proveedores_gastos", "proveedores",
        "flujo_caja", "detracciones_lotes", "detraccion_lotes", "ordenes_pago",
        "ordenes_servicio", "empleados", "categorias_gasto", "areas_gasto",
        "garantias", "movimientos_caja", "creditos_cliente", "prestamos",
        "configuracion_empresa", "configuracion_alertas", "configuracion_documentos",
        "cotizaciones", "ventas_comercial", "ordenes_cobro", "cuentas_bancarias",
        "conciliaciones_bancarias", "auditoria_logs",
    ]
    with engine.connect() as conn:
        for tabla in tablas:
            conn.execute(text(
                f"ALTER TABLE {tabla} "
                f"ADD COLUMN IF NOT EXISTS empresa_id INTEGER REFERENCES empresas(id)"
            ))
        conn.commit()
        # Columnas adicionales en tabla empresas (idempotente)
        for sql in [
...
app.add_middleware(TenantMiddleware)   # innermost: corre después de CORS
...
app.include_router(admin_router.router,                    prefix="/api/admin",                  tags=["Admin Panel"],      dependencies=[Depends(require_superadmin)])
