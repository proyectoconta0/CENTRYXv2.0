...
@router.get("/resumen")
def resumen_prestamos(db: Session = Depends(get_db)):
    def _suma(campo, tipos):
        return float(db.query(func.sum(campo)).filter(Prestamo.tipo.in_(tipos)).scalar() or 0)
...
def obtener_cronograma(prestamo_id: int, db: Session = Depends(get_db)):
    if not db.query(Prestamo).filter(Prestamo.id == prestamo_id).first():
        raise HTTPException(404, "Préstamo no encontrado")
    cuotas = db.query(CuotaPrestamo).filter(CuotaPrestamo.prestamo_id == prestamo_id) \
               .order_by(CuotaPrestamo.numero_cuota.asc()).all()
...
def _generar_gastos_financieros_cuota(db: Session, prestamo: Prestamo, cuota: CuotaPrestamo, usuario: Usuario) -> None:
...
def pagar_cuota_cronograma(prestamo_id: int, cuota_id: int, data: PagarCuotaCronogramaRequest, http_request: Request,
    ...
    _generar_gastos_financieros_cuota(db, p, cuota, usuario)
