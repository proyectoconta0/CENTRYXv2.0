...
def _serialize(vc: VentaComercial, cli: Cliente = None, pagado_con_garantia: bool = False,
                nota_credito_tipo: Optional[str] = None) -> dict:
    ...
    return {
        ...
        # Escenario de la última Nota de Crédito que referenció esta factura
        # (VentaComercial.tipo_nota_credito, ver comprobantes.py/_aplicar_nc) —
        # "anulacion_simple" | "devolucion_cobro" | "descuento_parcial" | None.
        # No se deriva de estado_cobranza a propósito (ver comentario sobre
        # METODOS_GARANTIA más arriba): ese campo no debe ganar valores nuevos.
        "nota_credito_tipo": nota_credito_tipo,
    }
...
def listar_cobranza(
    ...
    # Mapa factura_id -> tipo_nota_credito, resuelto en una sola query batched
    # (evita N+1) para los badges de NC en la tabla — ver _serialize.
    ids_pagina = [vc.id for vc, _ in page_rows]
    nc_tipo_map: dict = {}
    if ids_pagina:
        for factura_id, tipo_nc in db.query(
            VentaComercial.comprobante_relacionado_id, VentaComercial.tipo_nota_credito
        ).filter(
            VentaComercial.tipo_documento == "Nota de Crédito",
            VentaComercial.comprobante_relacionado_id.in_(ids_pagina),
        ).all():
            nc_tipo_map[factura_id] = tipo_nc

    return {
        "total":    total,
        "page":     page,
        "per_page": per_page,
        "data":     [_serialize(vc, cli, vc.id in ids_con_garantia, nc_tipo_map.get(vc.id)) for vc, cli in page_rows],
    }
