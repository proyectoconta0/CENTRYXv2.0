...
def _proximo_ac(db: Session, empresa_id=None) -> str:
    q = db.query(VentaComercial.numero_factura).filter(
...
def _proximo_ri_venta(db: Session, empresa_id=None) -> str:
...
def _proximo_correlativo_generico(db: Session, tipo_documento: str, prefijo: str, digitos: int = 5, empresa_id=None) -> str:
...
def crear(data: ComprobanteCreate, http_request: Request, db: Session = Depends(get_db),
          ...
          empresa_id: Optional[int] = Depends(get_empresa_id)):
    return _crear_comprobante(data, http_request, db, usuario, empresa_id=empresa_id)


def _crear_comprobante(data: ComprobanteCreate, http_request: Request, db: Session,
                        usuario: Usuario, empresa_id: Optional[int] = None) -> dict:
    ...
    if data.tipo_documento == "Anticipo de Cliente" and not numero_documento:
        numero_documento = _proximo_ac(db, empresa_id=empresa_id)
        ...
        numero_documento = _proximo_ri_venta(db, empresa_id=empresa_id)
        ...
        numero_documento = _proximo_correlativo_generico(db, "Nota de Crédito", "NC", empresa_id=empresa_id)
    elif data.tipo_documento == "Nota de Débito" and not numero_documento:
        numero_documento = _proximo_correlativo_generico(db, "Nota de Débito", "ND", empresa_id=empresa_id)

    q_dup = db.query(VentaComercial).filter(VentaComercial.numero_factura == numero_documento)
    if empresa_id is not None:
    ...
    if not data.archivo_temp:
        ...
        if data.tipo_documento in ("Nota de Crédito", "Nota de Débito"):
            ...
            q_fc = db.query(VentaComercial).filter(
                VentaComercial.numero_factura == data.documento_relacionado,
                VentaComercial.tipo_documento.in_(TIPOS_COBRANZA),
                VentaComercial.estado != "Anulada",
            )
            if empresa_id is not None:
                q_fc = q_fc.filter(VentaComercial.empresa_id == empresa_id)
            factura_check = q_fc.first()
            if not factura_check:
                ...
                if m_check:
                    ...
                    q_fc2 = db.query(VentaComercial).filter(
                        VentaComercial.numero_factura.ilike(f"%{serie_chk}%{numero_chk}"),
                        VentaComercial.tipo_documento.in_(TIPOS_COBRANZA),
                        VentaComercial.estado != "Anulada",
                    )
                    if empresa_id is not None:
                        q_fc2 = q_fc2.filter(VentaComercial.empresa_id == empresa_id)
                    factura_check = q_fc2.first()
            if not factura_check:
    ...
    if data.tipo_documento in ("Nota de Crédito", "Nota de Débito") and data.documento_relacionado:
        q_fac = db.query(VentaComercial).filter(
            VentaComercial.numero_factura == data.documento_relacionado,
            VentaComercial.tipo_documento.in_(TIPOS_COBRANZA),
            VentaComercial.estado != "Anulada",
        )
        if empresa_id is not None:
            q_fac = q_fac.filter(VentaComercial.empresa_id == empresa_id)
        factura = q_fac.first()
        if not factura:
            ...
            if m_ref:
                ...
                q_fac2 = db.query(VentaComercial).filter(
                    VentaComercial.numero_factura.ilike(f"%{serie}%{numero_sin_ceros}"),
                    VentaComercial.tipo_documento.in_(TIPOS_COBRANZA),
                    VentaComercial.estado != "Anulada",
                )
                if empresa_id is not None:
                    q_fac2 = q_fac2.filter(VentaComercial.empresa_id == empresa_id)
                factura = q_fac2.first()
...
def anular(comp_id: int, data: AnularRequest, http_request: Request, db: Session = Depends(get_db),
           usuario: Usuario = Depends(get_current_usuario),
           empresa_id: Optional[int] = Depends(get_empresa_id)):
    q = db.query(VentaComercial).filter(VentaComercial.id == comp_id)
    if empresa_id is not None:
        q = q.filter(VentaComercial.empresa_id == empresa_id)
    factura = q.first()
    if not factura:
    ...
    q_nc = db.query(VentaComercial).filter(VentaComercial.id == data.nota_credito_id)
    if empresa_id is not None:
        q_nc = q_nc.filter(VentaComercial.empresa_id == empresa_id)
    nota_credito = q_nc.first()
    if not nota_credito:
...
def _procesar_pdf_individual(contenido: bytes, nombre_archivo: str, db: Session,
    ...
    if resultado["numero_documento"]:
        q_ex = db.query(VentaComercial).filter(
            VentaComercial.numero_factura == resultado["numero_documento"]
        )
        if empresa_id is not None:
            q_ex = q_ex.filter(VentaComercial.empresa_id == empresa_id)
        existente = q_ex.first()
...
async def importar_pdf(file: UploadFile = File(...), db: Session = Depends(get_db),
    ...
    resultado = _procesar_pdf_individual(contenido, file.filename, db, empresa_id=empresa_id)
...
async def importar_zip(file: UploadFile = File(...), db: Session = Depends(get_db),
    ...
    for pdf_name in pdf_names:
        ...
        resultado = _procesar_pdf_individual(contenido_pdf, nombre_corto, db, empresa_id=empresa_id)
        ...
        if resultado["estado"] in ("listo", "revisar"):
            q_cfg = db.query(ConfiguracionEmpresa)
            if empresa_id is not None:
                q_cfg = q_cfg.filter(ConfiguracionEmpresa.empresa_id == empresa_id)
            empresa_cfg = q_cfg.first()
...
def importar_zip_confirmar(data: ImportarZipConfirmarRequest, http_request: Request,
    ...
    for item in data.comprobantes:
        try:
            r = _crear_comprobante(item, http_request, db, usuario, empresa_id=empresa_id)
            resultados.append({
...
def proximo_correlativo(tipo: str = "AC", db: Session = Depends(get_db),
    ...
    if tipo == "RI":
        return {"proximo": _proximo_ri_venta(db, empresa_id=empresa_id)}
    if tipo == "NC":
        return {"proximo": _proximo_correlativo_generico(db, "Nota de Crédito", "NC", empresa_id=empresa_id)}
    if tipo == "ND":
        return {"proximo": _proximo_correlativo_generico(db, "Nota de Débito", "ND", empresa_id=empresa_id)}
    return {"proximo": _proximo_ac(db, empresa_id=empresa_id)}
...
def obtener(comp_id: int, db: Session = Depends(get_db),
    ...
    q = db.query(VentaComercial, Cliente).outerjoin(
        Cliente, VentaComercial.cliente_id == Cliente.id
    ).filter(VentaComercial.id == comp_id)
    if empresa_id is not None:
        q = q.filter(VentaComercial.empresa_id == empresa_id)
    row = q.first()
    if not row:
...
def actualizar(comp_id: int, data: ComprobanteUpdate, http_request: Request, db: Session = Depends(get_db),
    ...
    if (documento_relacionado_cambio or tipo_cambio_relacion) and venta.tipo_documento in ("Nota de Crédito", "Nota de Débito"):
        ...
        if venta.documento_relacionado:
            ...
            q_frel = db.query(VentaComercial).filter(
                VentaComercial.numero_factura == venta.documento_relacionado,
                VentaComercial.tipo_documento.in_(TIPOS_COBRANZA),
                VentaComercial.estado != "Anulada",
            )
            if empresa_id is not None:
                q_frel = q_frel.filter(VentaComercial.empresa_id == empresa_id)
            factura_rel = q_frel.first()
            if not factura_rel:
                ...
                if m_put:
                    ...
                    q_frel2 = db.query(VentaComercial).filter(
                        VentaComercial.numero_factura.ilike(f"%{serie_put}%{numero_put}"),
                        VentaComercial.tipo_documento.in_(TIPOS_COBRANZA),
                        VentaComercial.estado != "Anulada",
                    )
                    if empresa_id is not None:
                        q_frel2 = q_frel2.filter(VentaComercial.empresa_id == empresa_id)
                    factura_rel = q_frel2.first()
            if not factura_rel:
                raise HTTPException(404,
