...
def pdf_orden_cobro(orden_id: int, db: Session = Depends(get_db),
    ...
    pdf_bytes = construir_pdf_orden_cobro(empresa, orden)
    return Response(
        content=pdf_bytes,
...
def eliminar_orden_cobro(orden_id: int, db: Session = Depends(get_db),
    ...
    # Limpia su espejo en movimientos_conciliacion (ver sincronizar_pagos_periodo)
    # antes de borrar la orden — si no, el movimiento 'sistema' quedaría
    # huérfano apuntando a un orden_id que ya no existe.
    limpiar_movimiento_sistema_por_pago(db, "orden_cobro", orden_id)
