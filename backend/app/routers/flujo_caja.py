...
def detalle_movimiento(mov_id: int, db: Session = Depends(get_db)):
    ...
    orden = db.query(OrdenCobro).filter(OrdenCobro.id == mov.referencia_sistema_id).first()
    if not orden:
...
def actualizar_conciliacion(id: int, db: Session = Depends(get_db)):
...
def reconciliar_movimiento(movimiento_id: int, db: Session = Depends(get_db)):
    ...
    conc = db.query(ConciliacionBancaria).filter(ConciliacionBancaria.id == mov.conciliacion_id).first()
