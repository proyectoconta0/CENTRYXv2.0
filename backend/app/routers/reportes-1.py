...
def _guardar_historial(db: Session, tipo: str, periodo_label: str, desde: date, hasta: date,
                        formato: str, filtros: dict, contenido: bytes, nombre_archivo: str,
                        empresa_id: Optional[int] = None) -> ReporteGenerado:
...
def reporte_general(
    ...
    empresa_header = get_empresa_header(db, para_pdf=True, empresa_id=empresa_id)
    color_emp = empresa_header["color_principal"]

    resumen  = svc.resumen_ejecutivo(db, d, h, empresa_id=empresa_id)
    ventas   = svc.datos_ventas(db, d, h, empresa_id=empresa_id)
    gastos   = svc.datos_gastos(db, d, h, empresa_id=empresa_id)
    cobranza = svc.datos_cobranza(db, d, h, empresa_id=empresa_id)
    pagos    = svc.datos_pagos_realizados(db, d, h, empresa_id=empresa_id)
    ...
    _guardar_historial(db, "general", label, d, h, formato, {}, contenido, nombre, empresa_id=empresa_id)
...
def reporte_ventas(
    ...
    empresa_header = get_empresa_header(db, para_pdf=True, empresa_id=empresa_id)
    color_emp = empresa_header["color_principal"]
    filas = svc.datos_ventas(db, d, h, cliente, tipo_servicio, tipo_documento, empresa_id=empresa_id)
    ...
    _guardar_historial(db, "ventas", label, d, h, formato, filtros, contenido, nombre, empresa_id=empresa_id)
    return _responder(contenido, formato, nombre)
...
def reporte_gastos(
    ...
    empresa_header = get_empresa_header(db, para_pdf=True, empresa_id=empresa_id)
    color_emp = empresa_header["color_principal"]
    filas = svc.datos_gastos(db, d, h, categoria, area, tipo_comprobante, proveedor, empresa_id=empresa_id)
    ...
    _guardar_historial(db, "gastos", label, d, h, formato, filtros, contenido, nombre, empresa_id=empresa_id)
    return _responder(contenido, formato, nombre)
...
def reporte_cobranza(
    ...
    empresa_header = get_empresa_header(db, para_pdf=True, empresa_id=empresa_id)
    color_emp = empresa_header["color_principal"]
    filas = svc.datos_cobranza(db, d, h, cliente, estado, empresa_id=empresa_id)
    ...
    _guardar_historial(db, "cobranza", label, d, h, formato, filtros, contenido, nombre, empresa_id=empresa_id)
    return _responder(contenido, formato, nombre)
...
def reporte_flujo_caja(
    ...
    empresa_header = get_empresa_header(db, para_pdf=True, empresa_id=empresa_id)
    color_emp = empresa_header["color_principal"]
    filas = svc.datos_flujo_caja(db, d, h, cuenta_bancaria_id, empresa_id=empresa_id)
    ...
    _guardar_historial(db, "flujo_caja", label, d, h, formato, filtros, contenido, nombre, empresa_id=empresa_id)
    return _responder(contenido, formato, nombre)
...
def reporte_proveedores(
    ...
    empresa_header = get_empresa_header(db, para_pdf=True, empresa_id=empresa_id)
    color_emp = empresa_header["color_principal"]
    filas = svc.datos_proveedores(db, d, h, proveedor, estado_pago, empresa_id=empresa_id)
    ...
    _guardar_historial(db, "proveedores", label, d, h, formato, filtros, contenido, nombre, empresa_id=empresa_id)
    return _responder(contenido, formato, nombre)
...
def historial(db: Session = Depends(get_db),
    ...
    if empresa_id is not None:
        q = q.filter(ReporteGenerado.empresa_id == empresa_id)
    rows = q.order_by(ReporteGenerado.id.desc()).limit(5).all()
...
def descargar_historial(id: int, db: Session = Depends(get_db),
                         empresa_id: Optional[int] = Depends(get_empresa_id)):
    q = db.query(ReporteGenerado).filter(ReporteGenerado.id == id)
    if empresa_id is not None:
        q = q.filter(ReporteGenerado.empresa_id == empresa_id)
    reg = q.first()
    if not reg:
