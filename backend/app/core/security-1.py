...
# Roles canónicos: Superadmin | Administrador | Vendedor.
# Superadmin: acceso total al panel de administración de empresas/usuarios.
# Se normalizan también los roles legacy (admin, gerente, contador).
...
ROLES_DISPONIBLES = ["Superadmin", "Administrador", "Vendedor"]
...
def require_administrador(usuario: Usuario = Depends(get_current_usuario)) -> Usuario:
    if normalizar_rol(usuario.rol) not in ("administrador", "superadmin"):
        raise HTTPException(status.HTTP_403_FORBIDDEN, "Solo un Administrador puede realizar esta acción")
    return usuario


def require_superadmin(usuario: Usuario = Depends(get_current_usuario)) -> Usuario:
    """Solo Superadmin puede acceder al panel de administración de empresas/usuarios."""
    if normalizar_rol(usuario.rol) != "superadmin":
        raise HTTPException(status.HTTP_403_FORBIDDEN, "Solo un Superadmin puede realizar esta acción")
    return usuario


# ── Multi-Tenant: empresa_id por request ──────────────────────────────────────

def get_empresa_id(request: Request) -> Optional[int]:
    """
    Retorna el empresa_id inyectado por TenantMiddleware, o None cuando:
      - Se accede desde localhost (desarrollo local, sin subdominio)
      - El subdominio es 'admin'
      - La ruta está en el bypass del middleware

    Usar como dependencia en endpoints:
        @router.get("/")
        def listar(empresa_id: Optional[int] = Depends(get_empresa_id)):
