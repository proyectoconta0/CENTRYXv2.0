"""
PDF de Orden de Pago (ReportLab, en memoria). Mismo lenguaje visual que
app/services/comprobante_pdf.py (franja de color de marca + badge + totales
destacados), con badge azul oscuro en vez de morado.
"""
import io
...
from app.models.models import OrdenPago

# Paleta azul oscura (Tailwind blue.800/900) — badge de Orden de Pago
PRIMARIO_HEX = "#1E40AF"   # blue.800
OSCURO_HEX   = "#1E3A8A"   # blue.900
CLARO_HEX    = "#EFF6FF"   # blue.50
BORDE_HEX    = "#BFDBFE"   # blue.200
GRIS_HEX     = "#6B7280"
...
def construir_pdf_orden_pago(empresa: dict, orden: OrdenPago) -> bytes:
    """orden: instancia de OrdenPago (con .detalles ya cargado vía relationship)."""
    buf = io.BytesIO()
    doc = SimpleDocTemplate(
        buf, pagesize=A4,
        leftMargin=2 * cm, rightMargin=2 * cm, topMargin=1.8 * cm, bottomMargin=1.8 * cm,
        title=f"Orden de Pago {orden.numero_orden or ''}",
    )
    ...
    badge_cell = Table([[Paragraph("ORDEN DE PAGO", badge_style)]], colWidths=[4.5 * cm])
    ...
    doc_info_flow = [
        ...
        Paragraph(f"Fecha: {_fmt_fecha(orden.fecha_pago)}", fecha_style),
    ]
    ...
    # ── PROVEEDOR ────────────────────────────────────────────────────────
    proveedor_tabla = Table([
        [Paragraph("PROVEEDOR:", label_style), Paragraph(orden.nombre_proveedor or "—", valor_style)],
        [Paragraph("RUC:", label_style), Paragraph(orden.ruc_proveedor or "—", valor_style)],
    ], colWidths=[3.2 * cm, 12.8 * cm])
    proveedor_tabla.setStyle(TableStyle([
    ...
    elementos.append(proveedor_tabla)
    elementos.append(Spacer(1, 0.5 * cm))
    ...
    for i, d in enumerate(orden.detalles, start=1):
        g = d.gasto
        numero_comprobante = (g.numero_comprobante if g else None) or "—"
        descripcion = (g.descripcion if g else None) or (g.categoria if g else None) or "Gasto"
        filas.append([str(i), numero_comprobante, Paragraph(descripcion, valor_style), _fmt_monto(d.monto_pagado, moneda)])
    ...
    # ── TOTAL ────────────────────────────────────────────────────────────
    total_tabla = Table([["TOTAL A PAGAR", _fmt_monto(orden.monto_total, moneda)]], colWidths=[11 * cm, 5 * cm], hAlign="RIGHT")
    ...
    # ── MÉTODO DE PAGO ───────────────────────────────────────────────────
    metodo_filas = [[Paragraph("MÉTODO DE PAGO:", label_style), Paragraph(orden.metodo_pago or "—", valor_style)]]
    if orden.banco:
    ...
    # ── FIRMAS: elaborado por / aprobado por ────────────────────────────
    ...
    firmas_tabla = Table([
        [Paragraph("ELABORADO POR:", label_style), Paragraph("APROBADO POR:", label_style)],
        [firma_izq, firma_der],
    ], colWidths=[8 * cm, 8 * cm])
