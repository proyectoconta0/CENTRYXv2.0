...
def datos_ventas(db: Session, desde: date, hasta: date, cliente: Optional[str] = None,
                  tipo_servicio: Optional[str] = None, tipo_documento: Optional[str] = None) -> list:
...
def datos_gastos(db: Session, desde: date, hasta: date, categoria: Optional[str] = None,
                  ...
                  proveedor: Optional[str] = None) -> list:
...
def datos_flujo_caja(db: Session, desde: date, hasta: date, cuenta_bancaria_id: Optional[int] = None) -> list:
    cuenta = None
...
def datos_proveedores(db: Session, desde: date, hasta: date, proveedor: Optional[str] = None,
                       estado_pago: Optional[str] = None) -> list:
...
def datos_pagos_realizados(db: Session, desde: date, hasta: date) -> list:
    rows = db.query(PagoGasto).filter(
        PagoGasto.fecha_pago >= desde, PagoGasto.fecha_pago <= hasta,
    ).order_by(PagoGasto.fecha_pago.desc()).all()
...
def resumen_ejecutivo(db: Session, desde: date, hasta: date) -> dict:
