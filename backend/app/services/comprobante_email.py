...
def descifrar_smtp_password(smtp_password: str) -> str:
    """Descifra smtp_password si fue guardada cifrada con Fernet
    (ver PUT /configuracion/empresa). Si FERNET_KEY no está configurada, o el
    valor no es un token Fernet válido (contraseña legacy guardada en texto
    plano antes de este cambio), se devuelve tal cual."""
    fernet_key = os.getenv("FERNET_KEY")
    if not fernet_key or not smtp_password:
        return smtp_password
    try:
        from cryptography.fernet import Fernet
        f = Fernet(fernet_key.encode())
        return f.decrypt(smtp_password.encode()).decode()
    except Exception:
        return smtp_password
...
def construir_cuerpo_html(empresa: dict, s: dict) -> str:
    ...
    <div style="font-family: Arial, sans-serif; font-size: 14px; color: #1f2937; line-height: 1.6;">
      ...
      <p style="margin-top: 24px;">
        Atentamente,<br>
        {empresa.get('nombre_empresa') or 'Centryx'}
      </p>
    </div>
    """.strip()
...
def enviar_email_smtp(
    *,
    ...
    with smtplib.SMTP(smtp_host, smtp_port, timeout=20) as server:
        server.starttls()
        server.login(smtp_usuario, descifrar_smtp_password(smtp_password))
